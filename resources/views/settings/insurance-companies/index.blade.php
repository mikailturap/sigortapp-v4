...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h2 mb-0 text-dark fw-normal">
            <i data-lucide="building" class="text-muted me-2" style="width: 24px; height: 24px;"></i>
            Sigorta Şirketleri
        </h1>
        <p class="text-muted mb-0 mt-1 small">Sigorta şirketlerini yönetin ve sıralayın</p>
    </div>
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('settings.insurance-companies.create') }}" class="btn btn-primary">
...
<div class="card shadow-sm border-0">
    <div class="card-header bg-white border-0 py-3">
        <div class="d-flex align-items-center">
            ...
            <h5 class="mb-0 fw-normal text-dark">Sigorta Şirketleri Listesi</h5>
        </div>
    </div>
            ...
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        ...
                        <th class="border-0">Şirket Adı</th>
                ...
                <tbody id="sortable-insurance-companies">
                    @forelse($insuranceCompanies as $company)
                    <tr data-id="{{ $company->id }}" class="sortable-item">
                        ...
                        <td class="align-middle">
                            <span class="text-muted">{{ $company->description ?: 'Açıklama yok' }}</span>
                        </td>
                        <td class="align-middle text-center">
                            <span class="badge bg-light text-dark">{{ $company->sort_order }}</span>
                        </td>
                        <td class="align-middle text-center">
                        ...
                        <td class="align-middle text-center">
                            <span class="badge bg-info">{{ $company->policies()->count() }} poliçe</span>
                        </td>
                        <td class="align-middle text-center">
                            <div class="btn-group btn-group-sm" role="group">
                                <a href="{{ route('settings.insurance-companies.edit', $company) }}" 
                                   class="btn btn-outline-primary btn-sm" title="Düzenle">
                                    <i data-lucide="edit" style="width: 14px; height: 14px;"></i>
                                </a>
                                
                                @if($company->is_active && $company->can_deactivate)
                                    <form action="{{ route('settings.insurance-companies.deactivate', $company) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-outline-warning btn-sm" 
                                                title="Pasife Al" onclick="return confirm('Bu sigorta şirketini pasife almak istediğinizden emin misiniz?')">
                                            <i data-lucide="eye-off" style="width: 14px; height: 14px;"></i>
                                        </button>
                                    </form>
                                @elseif(!$company->is_active)
                                    <form action="{{ route('settings.insurance-companies.activate', $company) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                ...
                                @if($company->can_delete)
                                    <form action="{{ route('settings.insurance-companies.destroy', $company) }}" 
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-outline-danger btn-sm" 
                                                title="Sil" onclick="return confirm('Bu sigorta şirketini silmek istediğinizden emin misiniz?')">
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-4">
                            <div class="text-muted">
                                ...
                                <p class="mb-0">Henüz sigorta şirketi eklenmemiş</p>
                                <a href="{{ route('settings.insurance-companies.create') }}" class="btn btn-primary btn-sm mt-2">
                                    İlk Şirketi Ekle
                                </a>
                            </div>
                        </td>
...
document.addEventListener('DOMContentLoaded', function() {
    const sortableList = document.getElementById('sortable-insurance-companies');
    ...
    if (sortableList) {
        new Sortable(sortableList, {
            ...
            onEnd: function(evt) {
                ...
                // Sıralama güncelle
                fetch('{{ route("settings.insurance-companies.update-order") }}', {
                    method: 'POST',
                    headers: {
